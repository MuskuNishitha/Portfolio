...
export default function Resume() {
  ...
  return (
    <div className={` min-h-screen py-[100px] transition-colors duration-300 ${isDarkMode ? "bg-bg-2" : "bg-gray-50"}`}>
      <HeaderBanner title={"Resume"} />
      <section id="resume" className={`py-[50px] overflow-hidden transition-colors duration-300 ${isDarkMode ? "bg-bg-2" : "bg-gray-50"}`}>
        <div className="container-custom">
          <motion.div
          ...
          >
            <motion.div variants={itemVariants}>
              <motion.div className="flex items-center gap-3 text-2xl font-extrabold mb-8" whileHover="hover">
                <motion.div
              ...
              {experiences.length > 0 ? (
                <motion.div className="relative pl-7" initial="hidden" animate="visible" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }}>
                  <motion.div className="absolute top-2 left-0 w-0.5 bottom-0 bg-gradient-to-b from-primary to-transparent origin-top" variants={timelineLineVariants} />
                  ...
                  {experiences.map((exp, i) => (
                    ...
                    >
                      ...
                      <motion.div className="text-xs font-semibold tracking-wide mb-1.5 uppercase" style={{ color: "var(--primary-3)" }} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.15 + 0.1 }}>
                        {exp.time || "N/A"}
                      </motion.div>

                      <motion.div className="text-lg font-bold mb-1" style={{ color: isDarkMode ? "white" : "var(--text-heading)" }} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.15 + 0.2 }}>
                        {exp.title || "Untitled"}
                      </motion.div>

                      <motion.div className="text-sm mb-2" style={{ color: "var(--text-muted)" }} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.15 + 0.3 }}>
                        {exp.place || "Unknown"}
                      </motion.div>

                      {exp.achievements && Array.isArray(exp.achievements) && exp.achievements.length > 0 && (
                        <motion.ul className="space-y-1.5 mt-3" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.15 + 0.4 }}>
            ...
            <motion.div variants={itemVariants}>
              ...
              {education.length > 0 ? (
                <motion.div className="relative pl-7 mb-12" initial="hidden" animate="visible" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }}>
                  <motion.div className="absolute top-2 left-0 w-0.5 bottom-0 bg-gradient-to-b from-primary to-transparent origin-top" variants={timelineLineVariants} />
                  ...
                  {education.map((edu, i) => (
                    ...
                    >
                      ...
                      <motion.div className="text-sm" style={{ color: "var(--text-muted)" }} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.15 + 0.3 }}>
                        {edu.place || "Unknown"}
                      </motion.div>

                      {edu.cgpa && (
                        <motion.div className="text-xs mt-1" style={{ color: "var(--primary-3)" }} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.15 + 0.35 }}>
                          {edu.cgpa}
                        </motion.div>
                      )}
                    </motion.div>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          {cert.year && (
                            <span className="inline-block mt-2 text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: isDarkMode ? "rgba(135, 80, 247, 0.1)" : "rgba(135, 80, 247, 0.08)", color: "var(--primary-3)" }}>
                              {cert.year}
                            </span>
                          )}
                        </div>
