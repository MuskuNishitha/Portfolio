...
export default function AboutUs() {
  ...
  return (
    <section
      id="about"
      className={`py-[100px] ${bgColor} relative overflow-hidden`}
    >
      <HeaderBanner title={"About Us"} />

      <div
        className={`absolute inset-0 ${isDarkMode ? "bg-[linear-gradient(rgba(135,80,247,0.04)_1px,transparent_1px),linear-gradient(90deg,rgba(135,80,247,0.04)_1px,transparent_1px)]" : "bg-[linear-gradient(rgba(0,0,0,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(0,0,0,0.03)_1px,transparent_1px)]"} bg-[length:60px_60px]`}
      />
      <div
        className={`absolute top-[-20%] right-[-10%] w-[700px] h-[700px] ${isDarkMode ? "bg-[radial-gradient(circle,rgba(135,80,247,0.1)_0%,transparent_70%)]" : "bg-[radial-gradient(circle,rgba(135,80,247,0.05)_0%,transparent_70%)]"} rounded-full animate-float`}
      />
      <div
        className={`absolute bottom-[-20%] left-[-10%] w-[600px] h-[600px] ${isDarkMode ? "bg-[radial-gradient(circle,rgba(135,80,247,0.08)_0%,transparent_70%)]" : "bg-[radial-gradient(circle,rgba(135,80,247,0.03)_0%,transparent_70%)]"} rounded-full animate-float [animation-delay:2s]`}
      />
      ...
      <div className="container-custom relative z-10">
        ...
        >
          {stats.map((stat, index) => (
            ...
            >
              ...
              <div className="text-3xl font-extrabold text-primary-3 mb-1">
                {stat.value}
              </div>
        ...
        <div className="grid lg:grid-cols-2 gap-[50px]">
          ...
          >
            <div className="mb-8">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {(profile.role || []).map((role, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1 bg-primary/10 border border-primary/30 rounded-full text-xs text-primary-3 font-medium"
                  >
                    {role}
                  </span>
                ))}
            ...
            <div
              className={`bg-gradient-to-br ${cardBg} border ${borderColor} rounded-2xl p-6 mb-8`}
            >
              <h3
                className={`text-lg font-bold ${textPrimary} mb-5 flex items-center gap-2`}
              >
            ...
            <div
              className={`bg-gradient-to-br ${cardBg} border ${borderColor} rounded-2xl p-6`}
            >
              <h3
                className={`text-lg font-bold ${textPrimary} mb-5 flex items-center gap-2`}
              >
                {/* <span className="text-primary-3">Contact</span> */}
                Contact Info
              </h3>
              <div className="space-y-3">
                <div
                  className={`flex items-center gap-3 p-3 ${isDarkMode ? "bg-bg-2/50" : "bg-gray-100"} rounded-xl`}
                >
                  <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center text-primary-3">
                    Mail:
                  </div>
                  <div>
                    ...
                    <a
                      ...
                      className={`text-sm ${textPrimary} hover:text-primary-3 transition-colors`}
                    >
                      {profile.email}
                    </a>
                  </div>
          ...
          >
            ...
            >
              <h3
                className={`text-lg font-bold ${textPrimary} mb-6 flex items-center gap-2`}
              >
              ...
              <div className="space-y-5">
                {skills.map((skill, idx) => (
                  ...
                  >
                    ...
                    <div
                      className={`relative h-2 ${isDarkMode ? "bg-bg-2" : "bg-gray-200"} rounded-full overflow-hidden`}
                    >
                      <motion.div
                ...
                {education.map((edu, idx) => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {edu.cgpa && (
                        <span
                          className={`text-xs font-medium ${textPrimary} bg-primary/20 px-2 py-0.5 rounded-full`}
                        >
                          {edu.cgpa}
                        </span>
                      )}
            ...
            {experience.map((exp, index) => (
              ...
              >
                <div className="flex flex-wrap justify-between items-start mb-4">
                  ...
                  <span
                    className={`text-xs ${textMuted} ${isDarkMode ? "bg-bg-2" : "bg-gray-100"} px-3 py-1 rounded-full`}
                  >
                    {exp.time}
                  </span>
                </div>
                <ul className="space-y-2">
                  {(exp.achievements || []).map((achievement, i) => (
                    <li
                      key={i}
                      className={`${textSecondary} text-sm flex items-start gap-2`}
                    >
